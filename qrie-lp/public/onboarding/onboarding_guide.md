...
# 2) Set your QOP SQS ARN and Account ID for THIS region
...
- `QrieReadOnly-<acct>` cross-account role with **SecurityAudit** permissions that the QOP account can assume for resource discovery and policy evaluation.
...
**Q: What permissions does the QrieReadOnly role have?**

A: The role uses AWS's managed **SecurityAudit** policy, which provides read-only access to security-related resources across all AWS services. This enables Qrie to discover your resources and evaluate security policies without any write permissions. The role includes additional security controls:
- External ID requirement (`qrie-${QopAccountId}-2024`)
- Principal ARN restriction (only QOP roles matching `qrie-*` pattern can assume it)
